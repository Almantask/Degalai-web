...
export async function startApp(root: HTMLElement): Promise<void> {
  ...
  function donateHtml(): string {
    return `<section class="panel donate-panel" aria-label="${escapeHtml(t("donate.title"))}">
      ...
      <a class="donate-option primary" href="${PAYPAL_DONATE_URL}" target="_blank" rel="noopener noreferrer">${escapeHtml(t("donate.paypal"))}</a>
      <a class="donate-option secondary" href="${GITHUB_SPONSORS_URL}" target="_blank" rel="noopener noreferrer">${escapeHtml(t("donate.github"))}</a>
    </section>`;
  }
